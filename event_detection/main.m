...
% load( 1 file 
dataPath = "E:\毕设\Foot_database\A1"; % 请确保此路径指向包含 P1_all.mat 等文件的文件夹
load(fullfile(dataPath, "P1_all.mat"));
...
% 检查 dataPath 中实际存在哪些参与者的合并文件
existing_persons = {};
for k = 1:length(persons)
    if exist(fullfile(dataPath, sprintf('%s_all.mat', persons{k})), 'file')
        existing_persons{end+1} = persons{k};
    else
        warning('未找到文件: %s，将跳过。', fullfile(dataPath, sprintf('%s_all.mat', persons{k})));
    end
end

if isempty(existing_persons)
    error('dataPath 中未找到任何参与者的 _all.mat 文件。请检查路径或先运行 NewDatasetCreation2.m');
end
...
for k = 1:length(persons)
    fprintf('正在从参与者 %s 提取事件和特征...\n', persons{k})
    load(fullfile(dataPath, sprintf('%s_all.mat', persons{k})))
