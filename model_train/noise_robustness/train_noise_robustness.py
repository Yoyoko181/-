...
def _scale_noise_to_snr(clean: np.ndarray, noise: np.ndarray, snr_db: float) -> np.ndarray:
    ...
    scale = math.sqrt((pc * target_ratio) / (pn + 1e-12))
    return (noise.astype(np.float32, copy=False) * float(scale)).astype(np.float32, copy=False)


def _make_gaussian_noise(length: int, seed: int) -> np.ndarray:
    rng = np.random.default_rng(seed)
    n = rng.standard_normal(int(length)).astype(np.float32, copy=False)
    return _normalize_1d(n)


def _make_impulse_noise(length: int, seed: int, prob: float, amp: float) -> np.ndarray:
    rng = np.random.default_rng(seed)
    n = np.zeros(int(length), dtype=np.float32)
    mask = rng.random(int(length)) < float(prob)
    if mask.any():
        signs = rng.choice(np.array([-1.0, 1.0], dtype=np.float32), size=int(mask.sum()), replace=True)
        n[mask] = signs * float(amp)
    return _normalize_1d(n) if np.any(n) else n


def _list_noise_mats(noise_root: Path, noise_glob: str) -> list[Path]:
    if not str(noise_root).strip():
        return []
    if not noise_root.exists():
        return []
    mats = sorted(noise_root.glob(noise_glob))
    mats = [p for p in mats if p.is_file() and p.suffix.lower() == ".mat"]
    return mats


def _sample_structural_noise_segment(noise_mats: list[Path], segment_len: int, seed: int) -> np.ndarray:
    if not noise_mats:
        return np.zeros(int(segment_len), dtype=np.float32)
    rng = np.random.default_rng(seed)
    mat_path = noise_mats[int(rng.integers(0, len(noise_mats)))]
    x = _load_signal_1d_from_mat(mat_path)
    x = _normalize_1d(x)
    if x.size < segment_len:
        out = np.zeros(int(segment_len), dtype=np.float32)
        out[: x.size] = x
        return out
    max_start = x.size - segment_len
    start = int(rng.integers(0, max_start + 1))
    return x[start : start + segment_len].astype(np.float32, copy=False)
...
def _build_noise(
    ...
    *,
    snr_db: float,
    noise_mats: list[Path],
    nonstruct: str,
    impulse_prob: float,
    impulse_amp: float,
    seed: int,
) -> np.ndarray:
    n_total = np.zeros_like(clean_seg, dtype=np.float32)
    if noise_mats:
        n_total = n_total + _sample_structural_noise_segment(noise_mats, clean_seg.size, seed=seed + 11)
    nonstruct = (nonstruct or "none").strip().lower()
    if nonstruct in {"gaussian", "gauss"}:
        n_total = n_total + _make_gaussian_noise(clean_seg.size, seed=seed + 23)
    elif nonstruct in {"impulse", "spike"}:
        n_total = n_total + _make_impulse_noise(clean_seg.size, seed=seed + 37, prob=impulse_prob, amp=impulse_amp)
    elif nonstruct in {"gaussian+impulse", "gauss+impulse", "mix"}:
        n_total = n_total + _make_gaussian_noise(clean_seg.size, seed=seed + 23)
        n_total = n_total + _make_impulse_noise(clean_seg.size, seed=seed + 37, prob=impulse_prob, amp=impulse_amp)
    n_total = _normalize_1d(n_total) if np.any(n_total) else n_total
    return _scale_noise_to_snr(clean_seg, n_total, snr_db=snr_db)
...
def _ensure_cwt_images_from_raw(
    ...
    *,
    ...
    subset_out_name = (out_subset or subset).strip()
    subset_out = out_root / dataset / subset_out_name
    ...
    print(f"检查缺失并补齐 CWT: {subset_out_name} -> {subset_out}", flush=True)
    for person_name, mats in people:
        ...
        for mat_path in mats:
            ...
            for i, seg in enumerate(segs):
                if noise_enabled:
                    n = _build_noise(
                        seg,
                        snr_db=noise_snr_db,
                        noise_mats=noise_mats,
                        nonstruct=nonstruct_noise,
                        impulse_prob=impulse_prob,
                        impulse_amp=impulse_amp,
                        seed=seed + i * 997,
                    )
                    seg = (seg.astype(np.float32, copy=False) + n).astype(np.float32, copy=False)
            ...
            if files_done % 20 == 0:
                print(f"生成进度 {subset_out_name}: files={files_done} images={images_done}", flush=True)

    print(f"完成补齐 {subset_out_name}: files_processed={files_done} images_written={images_done}", flush=True)
    return subset_out
...
def main() -> None:
    ...
    noise_root = Path(args.struct_noise_root) if str(args.struct_noise_root).strip() else Path()
    noise_mats = _list_noise_mats(noise_root, args.struct_noise_glob) if str(args.struct_noise_root).strip() else []
    noise_enabled = True
    noise_tag = _snr_suffix(args.noise_snr_db)
    nonstruct_tag = (args.nonstruct_noise or "none").strip().lower()
    struct_tag = "struct" if noise_mats else "nostruct"
    test_out_subset = f"{test_subset}__noise__{struct_tag}__{nonstruct_tag}__{noise_tag}"
    ...
    if not args.skip_generate:
        for subset in sorted(set(train_subsets + [test_subset])):
            _ensure_cwt_images_from_raw(
                ...
                noise_mats=noise_mats,
                nonstruct_noise=args.nonstruct_noise,
    ...
    test_root = cwt_out_root / args.dataset / test_out_subset
    ...
    if str(args.struct_noise_root).strip():
        print(f"Struct noise mats: {len(noise_mats)} from {str(noise_root)}", flush=True)
    print(f"Noise: snr_db={args.noise_snr_db} nonstruct={args.nonstruct_noise} test_out_subset={test_out_subset}", flush=True)
    ...
    results_dir = Path(__file__).resolve().parents[1] / "results" / "noise_robustness"
